...
**job_links[] contract (PR #481, fix PR #483, extended PR feat/candidate-status-per-job):** Both `get_company_saved_candidates()` and `get_company_saved_candidates_filtered()` return `job_links[]` per candidate alongside `job_titles[]`. Each entry: `{job_id: int, title: str, apply_date: ISO str | null, application_status: str | null, candidate_status: str | null}`. `apply_date` and `application_status` are sourced from `job_applications` via `LEFT JOIN` — both are `null` when the company linked the candidate to a job without the candidate having applied. `candidate_status` comes from `company_candidate_job_refs.candidate_status` — null until the company sets it. `job_titles[]` is kept for backward compat. **Source of job links is `company_candidate_job_refs` only — never `company_saved_candidates.job_id`.** The `_unlinked` filter on the frontend checks `data-job-links` length (from refs), not `updated.job_id` (from `company_saved_candidates`). **Critical column name: `job_applications.applied_at` (not `created_at`).** The batch-fetch LEFT JOIN must select `ja.applied_at` — selecting `ja.created_at` causes a PostgreSQL column-not-found error and returns HTTP 500. **The old `job_links[].status` key is permanently renamed to `application_status` — do not re-add the old key.**
