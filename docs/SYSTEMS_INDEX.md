...
**Applicant classify → job ref rule:** `_execClassify` in `company.main.js` always calls `POST /company/saved-candidates/{uid}?job_id={_appJobId}` when `_appJobId` is present — regardless of `wasSaved`. This ensures a `company_candidate_job_refs` row is added for the current job on every classify action. `ON CONFLICT DO NOTHING` on both tables makes repeated calls idempotent. Save is never called without `job_id` from this interface.
