...
**Details:** PR-1 — Additive Schema · PR-JOB — Soft Archive · PR-2 — Backfill + Dual-write (+ Bnd-1–Bnd-12 correction round) · `ARCHITECTURE_FOUNDATION.md` (قواعد F1–F13) · `ARCHITECTURE.md §66` (PR-1) · `ARCHITECTURE.md §66b` (PR-JOB) · `ARCHITECTURE.md §66c` (PR-2 + corrections) · اختبارات pr-1-01 إلى pr-1-10e في `test_post_comments.py` · 80 اختبار في `test_pipeline_integration.py` · 23 اختبار في `test_job_archive_integration.py` · **108 اختبار في `test_pipeline_backfill.py`** (§A–§L) · **123 اختبار في `test_pipeline_backfill_integration.py`** (§1–§42، PostgreSQL فعلي)

**Source of Truth (PR-2 additions + Bnd corrections):**
...
- `auth.py → _pipeline_upsert_entry(conn, *, ..., initial_event_reason=None)` — **SELECT FOR UPDATE** ثم INSERT · عند INSERT مع `initial_event_reason`: ينشئ initial stage event (from_stage=NULL) · عند ربط application_id: يُحدِّث `source='application'` تلقائياً · يُرفع ValueError عند تعارض application_id · داخل transaction
...
- `auth.py → pipeline_backfill_dry_run()` — تحليل read-only بـ LEFT JOINs · `conflicts_by_type` بـ 8 أنواع · `blocking_conflicts` bool (ليس count)
- `auth.py → run_pipeline_backfill(dry_run=False)` — backfill idempotent بـ advisory lock · فحص atomic blocking INSIDE lock · Pass-1 NULL CCJR: يبحث في job_applications أولاً؛ إذا لم يجد → `null_ccjr_without_application` conflict لا entry · Pass-2 DO UPDATE يشمل `source='application'` في SET clause · Pass-3 يحفظ legacy created_at و created_by=NULL
- `auth.py → _migrate_partial_unique_application_id()` — advisory lock + recheck duplicates + recheck identity mismatches + BlockingConflictError + CREATE INDEX + verify via pg_indexes · يُستدعى من `POST /admin/pipeline/migrate-index` فقط (ليس من startup)
- `auth.py → apply_job(...)` — `created_by=user_id` (ليس job_company_id) · `initial_event_reason='application_submitted'`
- `auth.py → update_application_status(...)` — `reason='application_status_changed'` في pipeline stage event
- `auth.py → promote_application_to_shortlist(...)` — **Option B (دائم)**: لا كتابة إلى `company_saved_candidates` · SELECT فقط لقراءة `general_status` (بلا FOR UPDATE) · `reason='application_shortlisted'`
- `auth.py → update_candidate_job_status(...)` — ensure-entry (upsert قبل stage update) · يبحث عن job_applications للـ triple · `candidate_status=None` بلا تطبيق → ValueError · `candidate_status=None` مع تطبيق → revert pipeline لـ app-derived stage · `reason='candidate_job_status_changed'`
