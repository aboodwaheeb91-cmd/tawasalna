...
### 19. Notifications System ✅ (Phases 0–10 complete · Phase 11 deferred · V2 complete PRs #447–#453 · P1 application_status_changed PR #455)
**Purpose:** User notifications for messages, job applications, verification status, profile activity. Includes hooks for comment / reply / mention / job-apply / follow / verify / application_status_changed actions. Unread badge in app header via polling. V2 Smart Aggregation for follow, job application, comment, and reply hooks.
...
**Details:** `docs/NOTIFICATIONS_PLAN.md` (full phased plan, Phase 0–11 + V1 Status + V2 Final Status + P1 implementation) · `ARCHITECTURE.md §49`
**Status:** Phases 0–10 ✅ complete (PRs #431–#440). Phase 11 (Real-time / Push) is intentionally deferred — requires WebSocket P0 security debt resolution + explicit user decision. V2 Smart Aggregation complete (V2-0 to V2-6, PRs #447–#453). V2-1: schema (7 new columns + partial index). V2-2: follow aggregation. V2-3: job application aggregation. V2-4: comment/reply aggregation. V2-5: UI badge in notifications.html. V2-6: Final QA. Mentions remain V1 (event_key idempotency). **application_status_changed ✅ implemented (PR #455)** — hook in `update_application_status(app_id, status, actor_id)`, event_key `application_status:{app_id}:{status}`, self-guard, per-status title/body, link=/job-detail. rating / job_expiring_soon remain P2 Missing.
**Do not recreate:** Do not create per-feature notification tables. Do not use `X-User-Id` header — JWT Bearer only. Do not use `innerHTML` with notification API data. Do not add a WebSocket route for notifications — Phase 11 requires explicit approval. `event_key` format: `{type}:{entity_type}:{entity_id}:{actor_id}` — all hooks must follow this pattern. `aggregation_key` format: `{kind}_agg:{target_type}:{target_id}` — do not expose on frontend or use to generate routes. `notification.link` always comes from backend — never generate routes from `aggregation_key` on the frontend.
