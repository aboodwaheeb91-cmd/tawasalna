...
1. `job_applications.status` — application lifecycle status tracking the candidate's progress through a job's hiring pipeline (`pending/viewed/accepted/contacted/interview/hired/rejected`). The company manages transitions; it is not purely applicant-driven. Modified by `update_application_status()` only. Never modified by the saved-candidates system.
2. `company_saved_candidates.status` — company's general pipeline classification of the candidate (one row per company-candidate pair). Modified by `PATCH /company/saved-candidates/{id}`.
3. `company_candidate_job_refs.candidate_status` — company's **per-job** classification of the candidate (one row per company-candidate-job triple). Modified by `PATCH /company/saved-candidates/{id}/jobs/{job_id}`. NULL means not yet classified for that job. Never auto-copied from either other field.
...
- **Card-level lock (`data-job-status-saving`):** set on card at PATCH start; prevents a second pick on the same card while one is in-flight. All `co-cand-job-status-dp .co-dp-btn` elements are disabled during the lock. `_toggleDpOf` respects `btn.disabled`. `_renderCandidateJobLinksUI` rebuilds pickers with `meta.locked=true` (buttons rendered `disabled`) while lock is active; `finally` removes the lock and re-enables the freshly rebuilt buttons.
...
- **Success path:** reads most-recent `data-job-links`, maps updated `candidate_status` for the matching `job_id`, calls `_renderCandidateJobLinksUI(card, links)`.
- **Failure/catch path:** reads `data-job-links` unchanged, calls `_renderCandidateJobLinksUI(card, rollLinks)` — UI reverts to last known good state without mutating `data-job-links`. No `data-prev-val` fallback — the rebuild from `data-job-links` is the rollback.
...
- **No async path reads a DOM element captured before the PATCH call** — `wrap` is read at click time (guaranteed live); `card` is captured from `wrap.closest()` before async; `data-job-links` is read from `card` at response time (always fresh).
