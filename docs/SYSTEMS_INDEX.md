...
**Details:** PR-1 — Additive Schema · PR-JOB — Soft Archive · PR-2 — Backfill + Dual-write (+ Bnd-1–Bnd-12 + Bnd-R2-1–Bnd-R2-8 correction rounds) · `ARCHITECTURE_FOUNDATION.md` (قواعد F1–F13) · `ARCHITECTURE.md §66` (PR-1) · `ARCHITECTURE.md §66b` (PR-JOB) · `ARCHITECTURE.md §66c` (PR-2 + تصحيحات دور 1 و2) · اختبارات pr-1-01 إلى pr-1-10e في `test_post_comments.py` · 80 اختبار في `test_pipeline_integration.py` · 23 اختبار في `test_job_archive_integration.py` · **129 اختبار في `test_pipeline_backfill.py`** (§A–§M) · **135 اختبار في `test_pipeline_backfill_integration.py`** (§1–§47، PostgreSQL فعلي) · **27 اختبار في `test_pipeline_backfill_http.py`** (TestClient، لا DB)
