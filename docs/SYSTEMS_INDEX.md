...
- Popover (3 rows): Row 1 = حالة الطلب (`data-app-status` / `application_status` from `job_applications`); Row 2 = تصنيف في الوظيفة (`data-cand-status` / `candidate_status` from `company_candidate_job_refs`); Row 3 = التصنيف العام (`data-status` from card / `company_saved_candidates.status`).
