...
**Purpose:** 1-level reply threading per-post comments on company posts. Auth required to create/edit/delete. Server-side permission enforcement. Soft delete. XSS-safe rendering via `_renderCommentBody(bodyEl, text, mentionName)`. `comments_count` returned with every post. Reply threading: `reply_to_comment_id` nullable FK, max depth=1 enforced server-side. Replies grouped under parent on load (`_cmtRenderComments`), inserted after last sibling on send (`_cmtInsertReply`). `.pc-cmt-visual-reply` class driven by `reply_to_comment_id` (not body `@`). Portal ⋮ menu (`position:fixed` on `body`), auto-resize textarea, relative Arabic time, outlined send button, column header layout, scrollbar hidden (RTL vertical line fix). @ mention autocomplete: portal `#pc-cmt-mention-menu` on body, candidates from DOM (`.pc-cmt-author` textContent + company name), max 6, XSS-safe (textContent only), keyboard (Arrow/Enter/Escape), closes on outside click + scroll.
**Source of Truth:** `company_post_comments` table (incl. `reply_to_comment_id` column) · `auth.py → get_company_post_comments / create_company_post_comment / update_company_post_comment / delete_company_post_comment` · `GET /company/posts/{post_id}/comments` · `POST/PATCH/DELETE /company/posts/comments/{comment_id}` · `static/company/company.posts.js` (`_toggleCommentPanel`, `_cmtBuildItem`, `_renderCommentBody`, `_cmtRenderComments`, `_cmtInsertReply`, `_cmtUpdateCount`, `_autoResizeTextarea`, `_formatRelativeTime`, `_cmtShowPortalMenu`, `_cmtHidePortalMenu`, `_cmtHandleReply`, `_cmtCancelReply`, `_cmtReplyTarget`, `_cmtReplyTargetId`, `_cmtMentionState`, `_cmtCollectMentionCandidates`, `_cmtOpenMentionMenu`, `_cmtCloseMentionMenu`, `_cmtInsertMention`, `_cmtFilterMentionCandidates`, `_cmtFindMentionStart`, `_cmtHandleMentionInput`, `_cmtHandleMentionKeydown`) · `static/company/company.css` (`.pc-cmt-mention-menu`, `.pc-cmt-mention-item`, `.pc-cmt-mention-active`)
**Details:** `CLAUDE.md → Post Comments System Rules` · `ARCHITECTURE.md §65`
**Do not recreate:** Max depth=1 — server resolves deeper replies to root parent. Do NOT create a separate reply endpoint — same `POST /comments` accepts `reply_to_comment_id`. `_cmtRenderComments` must be used for initial render (not `forEach`). `.pc-cmt-visual-reply` driven by `reply_to_comment_id != null`, NOT by body `@`. `_renderCommentBody(bodyEl, text, mentionName)` handles compound author names — do NOT use `\S+`-only match when `mentionName` is available. Delete is always soft. `viewer_can_edit` / `viewer_can_delete` from server only. Edit flow: `editWrap` inserted before `bodyEl` is hidden; `_cmtEditInFlight` guards PATCH; visual-reply class NOT changed in edit (immutable). ⋮ menu portal pattern — do NOT revert to inline. "رد" below `.pc-cmt-body`. "تم التعديل" → `.pc-cmt-meta-row`. Scrollbar hidden on `.pc-cmts-list`. Mention autocomplete: one portal `#pc-cmt-mention-menu` on body (do NOT duplicate); candidates from DOM textContent only (no API call); `innerHTML` forbidden for candidate names.
...
*Last updated: 2026-07-07 — reflects systems as of PR #386–#394 and feat/reply-threading-v1 + feat/comment-mention-autocomplete. §22c updated: reply_to_comment_id column (max depth=1), _cmtRenderComments groups replies under parent, _cmtInsertReply inserts after last sibling, _renderCommentBody compound @mention, _cmtReplyTargetId, visual-reply driven by reply_to_comment_id. Mention autocomplete: portal #pc-cmt-mention-menu, DOM-based candidates, XSS-safe textContent, keyboard nav.*
