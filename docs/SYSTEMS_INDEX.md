...
1. `job_applications.status` — applicant-driven application status (`pending/viewed/accepted/contacted/interview/hired/rejected`). Set by company via the classify flow. Never modified by the saved-candidates system.
...
**Privacy:** Returns `candidate_id, tw_id, full_name, avatar_url, profession, city, country, job_id, status, notes, created_at, updated_at, job_titles[], job_links[]` only. `job_titles[]` = job title strings from `company_candidate_job_refs → jobs` (backward compat). `job_links[]` = rich objects `{job_id, title, apply_date, application_status, candidate_status}` from same refs — see **job_links[] contract** below. `job_id` in `company_saved_candidates` is legacy first-save origin (never overwritten, never shown in UI). Never returns email, phone, or KYC data.
**Pipeline statuses:** `saved | shortlisted | contacted | interview | hired | rejected`. Validated via `VALID_CANDIDATE_STATUSES` (shared for all three status sources). `notes` max 500 chars. Per-job accepted state is tracked via `job_applications.status='accepted'` (returned as `per_job_accepted` field when `?job_id=` filter active).
...
- Frontend: each job chip in manage panel has `.co-cand-job-status-sel` native `<select>`. `_onSavedChange` delegated handler auto-saves on change. On success, updates `data-cand-status` on the chip so the 3-row popover reflects fresh data immediately.
