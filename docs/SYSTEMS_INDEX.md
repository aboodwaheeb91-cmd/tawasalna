...
**Guest Menu Contract:** Login → `/login`. Register (إنشاء حساب) → `/login#register`. These MUST differ — clicking Register opens the registration form directly via hash routing in `index.ui.js`.
**index.auth.js Login Path Contract (T15–T16):** `doLogin()` uses allowlist writes only — removes `tw_user`/`tw_jwt`, then sets them; never touches unrelated keys (`tw_cover_*`, `tw_prefs`, `app_theme`). On storage quota failure (catch block): rolls back both `tw_user` and `tw_jwt` so neither persists. Verified by T15/T15b/T15c (success path preserves unrelated keys) and T16/T16b (rollback on `QuotaExceededError`).
...
### 49. Operational Feedback System V1 [DS-FEEDBACK]
**Purpose:** الـ contract المعماري الرسمي لـ Operational Feedback (Snackbar / Toast) في منصة تواصلنا — نظام الـ feedback المرتبط بعمليات المستخدم (حفظ / إرسال / تحديث / حذف). يُعرِّف Single Global Feedback Surface، Latest Replaces Current، 4 أنواع (success / error / warning / info)، Duration Policy مركزية، Mobile Behavior Contract (`--tw-feedback-bottom`)، RTL Centering (left:50% + translateX(-50%))، Layer Architecture (Conceptual Level 4)، Accessibility V1 (role="status" + polite)، Pointer Interaction (Message Body = pointer-events:none)، Error Normalization Ownership، DS-VAL × DS-FEEDBACK Boundary، XSS P0 Runtime Debt، Migration Inventory (4 categories)، 16 Runtime Debts، Must-Have V1 (19 بنداً)، Forbidden Patterns (30+).
**Source of Truth:** `docs/design-system/FEEDBACK-SYSTEM.md` (FBK-00 → FBK-29) · `docs/DESIGN_SYSTEM.md` (فهرس النظام) · `ARCHITECTURE_FOUNDATION.md` F34
**Details:** `docs/design-system/FEEDBACK-SYSTEM.md` — 30 قسماً: FBK-00 (Routing Protocol) → FBK-29 (Changelog). أبرز قواعد:
- **Routing Protocol** (FBK-00): جدول قرار — Form context → DS-VAL VAL-09 · Action context → DS-FEEDBACK · Navigation Snackbar → ممنوع مع twNavigate · Notification → Notification System.
- **Single Global Surface** (FBK-02): سطح Snackbar واحد عالمي في كل الصفحات. لا Stack/Queue V1.
- **4 أنواع رسمية** (FBK-04): success / error / warning / info — كلها مدعومة في V1.
- **Duration Policy** (FBK-07): `FBK_DURATION = { success: 2800, info: 3200, warning: 4000, error: 4500 }`. مركزية.
- **Lifecycle** (FBK-08): hidden → entering → visible → exiting → hidden. Stuck State Guard (1000ms timeout).
- **Mobile Behavior Contract** (FBK-09): `--tw-feedback-bottom` CSS custom property — لا أرقام hardcoded.
- **RTL Centering** (FBK-10): `left: 50%; transform: translateX(-50%)` — physical. ممنوع `inset-inline-start`.
- **Layer Architecture** (FBK-11): Conceptual Level 4. z-index:9999 = placeholder حتى Global Layer Tokens.
- **Accessibility** (FBK-12): جميع أنواع V1: `role="status"` + `aria-live="polite"` + `aria-atomic="true"`.
- **Pointer Interaction** (FBK-13): Message Body = `pointer-events: none` V1. Action Zone V2 = `pointer-events: auto`.
- **Operation Identity** (FBK-15): V1 Policy: Loading يبقى في Button/Component، Snackbar يعرض النتيجة فقط. V2 Concept: `updateFeedback(id, opts)` مؤجَّل.
- **Error Normalization** (FBK-19): DS-FEEDBACK يستقبل رسالة آمنة — لا يُحلِّل API body. chain: API-MUT-11 → Feature → DS-FEEDBACK.
- **DS-VAL × DS-FEEDBACK Boundary** (FBK-20): Form context → DS-VAL VAL-09; Action context → DS-FEEDBACK. Orchestration تقرر.
- **XSS P0 Runtime Debt** (FBK-21): `tw_shared.js:22` `innerHTML` مع dynamic msg — مُوثَّق، Runtime Fix يحتاج PR مستقل.
- **Migration Inventory** (FBK-23): 6 implementations موثَّقة + alert() → 4 categories (A=Operational Success / B=General Error / C=Validation → DS-VAL / D=Display → DS-OVL).
- **Runtime Debts** (FBK-24): M0 (XSS P0) → M15 — 16 ديناً موثَّقاً.
- **Must-Have V1** (FBK-25): 19 بنداً. **Defer V2+** (FBK-26): 10 بنود.
- **Forbidden Patterns** (FBK-27): 30+ بنداً يشمل innerHTML / role="alert" V1 / loading في Snackbar / dur=99999 / z-index hardcoded / inset-inline-start:50% / DS-FEEDBACK Runtime بدون موافقة.
**Do not recreate:** لا تُنشئ toast implementation مستقلة داخل أي page module — DS-FEEDBACK يملك هذا. لا `innerHTML` للـ msg. لا `role="alert"` في V1 (polite فقط). لا loading في Snackbar V1. لا hardcode z-index في Feature layer. لا `inset-inline-start: 50%` على Snackbar المُمركَز. لا `dur=99999` كبديل loading. لا DS-FEEDBACK Runtime حتى يُطلب صراحةً.
**Cross-references:** DS-VAL §44 VAL-09 (Form-level error banner — بديل DS-FEEDBACK في Form context). API-MUT §45 API-MUT-11 (`normalizeErrorResponse()` — العميل الإلزامي قبل DS-FEEDBACK). DS-OVL §48 OVL-14 (Layer Architecture Level 4 — Global Feedback Band). DS-NAV §41 (Navigation — ممنوع Snackbar مع twNavigate الفوري).
**Status:** V1 Documentation ✅ (2026-07-24) · Runtime Implementation 🔜 · Legacy Migration 🔜
...
**Details:** `docs/design-system/COLOR-SYSTEM.md` (CLR-00 → CLR-34, 35 قسماً). أبرز قواعد:
...
- **Semantic ≠ Categorical** (CLR-13): `--color-status-warning` إشارة UX (خلل/تحذير). `--color-categorical-amber` تمييز بيانات (مستوى مهارة/فئة). مستخدَم مختلف، سياق مختلف — ممنوع الخلط بينهما.
- **Domain/Feature Mapping Policy** (CLR-14): DS-COLOR يملك القيمة والهوية. Feature/Domain يملك المعنى (Tier 2 alias في feature CSS يُشير إلى DS-COLOR، لا يُعرِّف قيمة hex بنفسه).
- **Three-Tier Local Token Policy** (CLR-15): T1 = Global DS-COLOR في `tw_shared.css` · T2 = Domain Alias في feature CSS (`--co-accent: var(--color-brand-secondary)`) · T3 = Local non-color tokens + Local Color Role مسموح بشروط (documented exception). T2 ممنوع أن يُحدِّد قيم hex مستقلة. Shared Meaning هو معيار الترقية — ليس عدد الـ surfaces وحده (CLR-26).
- **company.css Migration Intent** (CLR-16): `company.css` يُعيد تعريف `--ac` و`--ac2` (دين معماري، ليس visual bug). الحل المخطط: domain alias `--co-accent: var(--color-brand-secondary)`. لا تعديل في Phase 0 أو Phase 1 basic.
...
**Details:** `docs/CHANGE_ROUTER.md` — Workflow Order · CRS-01 (A: Scope Detection → B: 17 Change Types → C: Primary Owner Resolution → D: Supporting Owner Resolution → E: Required Reading → F: Architectural Check → G: Impact Matrix → H: Verdict → I: Execution Scope) · CRS-02 (Sanity Check) · System Gap Contract · Shared System First (F30-based) · CRS-03 (TINY/MEDIUM/ARCHITECTURAL) · Context Reuse · Ambiguity/Confidence · Impact Matrix · Mobile/API-First Guard · Notification Routing · User Intent ≠ Implementation Method · Audit/Execution Modes · No Auto-Fix · Standard Output Format · Mandatory Architectural Opinion
**Do not recreate:** لا تُنشئ ملف "routing" أو "change-guide" مستقل. لا تُعيد نقل عقود الأنظمة إلى CHANGE_ROUTER.md — الـ Routing Protocols تبقى داخل كل نظام (BTN-00/CLR-00/…). CRS لا يُعدِّل Runtime تلقائياً. Workflow order: `ARCHITECTURE_FOUNDATION → SYSTEMS_INDEX → CRS → Governing System → Runtime` — Authority يبقى دائماً لـ ARCHITECTURE_FOUNDATION.
**Cross-references:** `ARCHITECTURE_FOUNDATION.md` F30 + F31 (القواعد التي يُطبِّقها CRS) · `docs/SYSTEMS_INDEX.md` (الفهرس الرسمي لتحديد المالك).
