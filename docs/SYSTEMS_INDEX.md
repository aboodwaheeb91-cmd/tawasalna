...
**job_links[] contract (PR #481):** Both `get_company_saved_candidates()` and `get_company_saved_candidates_filtered()` return `job_links[]` per candidate alongside `job_titles[]`. Each entry: `{job_id: int, title: str, apply_date: ISO str | null, status: str | null}`. `apply_date` and `status` are sourced from `job_applications` via `LEFT JOIN` — both are `null` when the company linked the candidate to a job without the candidate having applied. `job_titles[]` is kept for backward compat. **Source of job links is `company_candidate_job_refs` only — never `company_saved_candidates.job_id`.** The `_unlinked` filter on the frontend checks `data-job-links` length (from refs), not `updated.job_id` (from `company_saved_candidates`).
