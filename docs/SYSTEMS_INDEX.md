...
- **Popover (2 rows — simplified fix/job-chip-pop-simplify):** Row 1 = «حالة المرشح في هذه الوظيفة» — source: `data-cand-status` / `company_candidate_job_refs.candidate_status`; null → «غير مصنف». Row 2 = «تاريخ التقدم» — source: `data-apply-date` / `job_links[].apply_date`; shown **only when non-null** (candidate applied). `application_status` and `company_saved_candidates.status` are NOT shown in this popover — they remain in `job_links[]` API contract and the manage panel respectively. Do NOT re-add حالة الطلب or التصنيف العام rows to this popover.
