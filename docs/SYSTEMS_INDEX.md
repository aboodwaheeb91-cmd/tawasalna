...
### 23. Appointments & Interview Rooms System 🔜 planned — Phase 0 (Architecture Documented)
...
**Status:** 🔜 planned — architecture documented (PR #459, docs-only). No schema, no migration, no API, no frontend implemented yet. Scheduler-based reminders deferred to Phase 8.
**Source of Truth:** `docs/APPOINTMENTS_PLAN.md` — full 15-section architecture plan (purpose, core rule, user flows, UI cards, room structure, 9 states, response deadline, data model proposal, API proposal, permissions, event types, notifications, security risks, build phases, non-goals).
**Details:** `docs/APPOINTMENTS_PLAN.md` — the canonical reference for all future implementation phases (Phase 1–8).
**Related systems:** §14 (Jobs) · §15 (Job Applications) · §18 (Messaging — Messenger العام separate from Appointment Room) · §19 (Notifications — 7 event-driven hooks documented; 3 reminder hooks deferred to scheduler) · §2 (JWT Bearer token — only approved auth method)
**Do not recreate:**
- لا تُنشئ جدول مواعيد مختلف. مخطط الجداول موثق في `docs/APPOINTMENTS_PLAN.md §8` — لا تُغيّره إلا بموافقة معمارية.
- لا تدمج Appointment Room مع Messenger العام — نظامان منفصلان.
...
- لا تبدأ Phase 1 (schema only) إلا بطلب صريح من المستخدم.
