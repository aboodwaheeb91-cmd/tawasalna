...
**Purpose:** 1-level reply threading per-post comments on company posts. Auth required to create/edit/delete. Server-side permission enforcement. Soft delete. XSS-safe rendering via `_renderCommentBody` (7-arg). `comments_count` returned with every post. Reply threading: `reply_to_comment_id` nullable FK, max depth=1 enforced server-side. Replies are **collapsed by default** inside `.pc-cmt-replies-box` behind a toggle (`_cmtBuildRepliesGroup`, `_cmtSetToggleState`). Long comment bodies are **collapsed to 2 lines** via `is-collapsed` + `_cmtCheckCollapse`. Free @mentions from the autocomplete dropdown are stored in DB (`mentioned_tw_id`) and rendered as `<a href="/u/tw_id">` when present. `.pc-cmt-visual-reply` driven by `reply_to_comment_id`. Portal ⋮ menu, auto-resize textarea, relative Arabic time. **Author links:** avatar + name are `<a href="/u/{author_tw_id}">`. **@mention rendering priority:** (1) reply-author → `<a>` when `reply_to_author_tw_id` present · (2) DB-backed free mention → `<a>` when `mentioned_tw_id` present · (3) knownNames compound match → `<span>` · (4) `@\S+` → `<span>` · (5) plain text. **Mention dropdown:** portal `#pc-cmt-mention-menu`, candidates `{name, tw_id, avatar}`, max 6, keyboard nav, `visibility:hidden` measurement trick.
**Source of Truth:** `company_post_comments` table (incl. `reply_to_comment_id`, `mentioned_tw_id` columns) · `auth.py → get_company_post_comments / create_company_post_comment(…, mentioned_tw_id=None) / update / delete` · `GET /company/posts/{post_id}/comments` · `POST/PATCH/DELETE /company/posts/comments/{comment_id}` · `static/company/company.posts.js` (`_renderCommentBody` 7-arg, `_cmtBuildItem`, `_cmtBuildRepliesGroup`, `_cmtSetToggleState`, `_cmtReplyCountText`, `_cmtCheckCollapse`, `_cmtInitCollapseAll`, `_cmtInsertMention(ta, name, twId)`, `_cmtMentionedTwId`, `_cmtKnownNames`) · `static/company/company.css` (`.pc-cmt-body.is-collapsed`, `.pc-cmt-more-btn`, `.pc-cmt-less-btn`, `.pc-cmt-replies-toggle`, `.pc-cmt-replies-box`, `.pc-cmt-mention-*`, `a.pc-cmt-author`, `a.pc-cmt-ava`, `a.pc-cmt-mention`)
**Details:** `CLAUDE.md → Post Comments System Rules` · `ARCHITECTURE.md §65`
**Do not recreate:** Max depth=1 — server resolves deeper replies to root. Do NOT create a separate reply endpoint. `_renderCommentBody` has 7 args — always pass all (null for absent). `mentioned_tw_id` must be server-validated (user must exist in `users` table). Replies go into `.pc-cmt-replies-box` — never inline in list. `_cmtSetToggleState` is the ONLY function that changes toggle text + box visibility. `_cmtCheckCollapse` MUST be called after every comment insertion. Author/mention links ALWAYS `/u/{tw_id}`. `innerHTML` forbidden for any API text.
...
*Last updated: 2026-07-08 — reflects systems as of PR #386–#397 + feat/comment-ux-v2. §22c updated: 7-arg _renderCommentBody, DB-backed free @mention (mentioned_tw_id), collapsible long comments (_cmtCheckCollapse, is-collapsed), collapsed replies by default (_cmtBuildRepliesGroup, _cmtSetToggleState, _cmtReplyCountText), _cmtInsertMention(ta, name, twId).*
