...
**Status:** Phases 0–10 ✅ complete (PRs #431–#440). Phase 11 (Real-time / Push) is intentionally deferred — requires WebSocket P0 security debt resolution + explicit user decision. V2 Smart Aggregation Plan (V2-0) documented in PR #447 — implementation phases V2-1 to V2-6 are planned but not started.
**Do not recreate:** Do not create per-feature notification tables. Do not use `X-User-Id` header — JWT Bearer only. Do not use `innerHTML` with notification API data. Do not add a WebSocket route for notifications — Phase 11 requires explicit approval. `event_key` format: `{type}:{entity_type}:{entity_id}:{actor_id}` — all hooks must follow this pattern. Do not start V2 aggregation phases without explicit user approval — see `docs/NOTIFICATIONS_PLAN.md §V2`.
...
### 36. Notifications Full Delivery Plan ✅ V1 complete · V2 planning started (PR #447)
**Purpose:** الخطة المرحلية الكاملة لنظام الإشعارات (Phase 0–11 + V2 Aggregation Plan). توثّق الـ audit، الثغرات المحلولة (S1–S6)، وكل phase منفَّذ من PRs #431–#440، وخطة V2 للتجميع الذكي.
**Source of Truth:** `docs/NOTIFICATIONS_PLAN.md`
**Details:** `docs/NOTIFICATIONS_PLAN.md` — يتضمن: audit الموجود، حلول S1–S6، event_key format، hooks لكل نوع (comment/reply/mention/follow/job_applied/verify)، pagination، unread badge، Notifications V1 Status table، Phase 11 deferral conditions، **V2 Smart Aggregation Plan (V2-0 to V2-6)**.
**V2 Aggregation:** خطة توثيقية فقط (V2-0 = PR #447). تشمل: Follow Aggregation، Job Application Aggregation per job، Comment/Reply Aggregation per post، Helper مقترح `create_or_update_aggregated_notification`، Option A (aggregate while unread)، Click Target Rules، Anti-spam Rules. لا تنفيذ حتى يُطلب صراحةً.
**Do not recreate:** لا تبني نظام إشعارات موازياً. كل hook جديد يتبع pattern الـ event_key الموثق في §2. لا WebSocket للإشعارات قبل حل P0 Security Debt + قرار صريح من المستخدم. لا تبدأ V2 phases قبل طلب صريح.
