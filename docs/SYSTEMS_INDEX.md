...
**Purpose:** Flat (V1) per-post comments on company posts. Auth required to create/edit/delete. Server-side permission enforcement. Soft delete. XSS-safe rendering via `_renderCommentBody`. `comments_count` returned with every post. Auto-resize textarea, relative Arabic time, outlined send button, portal ⋮ menu (`position:fixed` on `body` — avoids overflow clip), flat reply with `@mention` prefill, "رداً على" strip, column header layout, visual reply indentation (`.pc-cmt-visual-reply`), scrollbar hidden (RTL vertical line fix).
**Source of Truth:** `company_post_comments` table · `auth.py → get_company_post_comments / create_company_post_comment / update_company_post_comment / delete_company_post_comment` · `GET /company/posts/{post_id}/comments` · `POST/PATCH/DELETE /company/posts/comments/{comment_id}` · `static/company/company.posts.js` (`_toggleCommentPanel`, `_cmtBuildItem`, `_renderCommentBody`, `_cmtUpdateCount`, `_autoResizeTextarea`, `_formatRelativeTime`, `_cmtShowPortalMenu`, `_cmtHidePortalMenu`, `_cmtHandleReply`, `_cmtCancelReply`)
**Details:** `CLAUDE.md → Post Comments System Rules` · `ARCHITECTURE.md §65`
**Do not recreate:** V1 is flat only — no nested replies, no parent_comment_id. Body always rendered via `_renderCommentBody()` — never `innerHTML` for API data. Delete is always soft (`status='deleted'`). `viewer_can_edit` / `viewer_can_delete` flags come from the server — never compute permissions on the frontend. Edit flow: `editWrap` inserted into DOM before `bodyEl` is hidden; `_cmtEditInFlight` guards concurrent PATCH. ⋮ menu uses portal pattern (`position:fixed`) — do NOT revert to inline. "رد" button is below `.pc-cmt-body`, not in `.pc-cmt-meta-row`. "تم التعديل" badge → `.pc-cmt-meta-row`. Scrollbar hidden on `.pc-cmts-list` — do NOT remove (RTL vertical line fix). `.pc-cmt-visual-reply` = CSS-only indentation for `@`-starting comments — no DB change.
...
*Last updated: 2026-07-07 — reflects systems as of PR #386–#393 and feat/comment-ux-polish-3. §22c updated: "رد" button moved below body, _renderCommentBody XSS-safe renderer, .pc-cmt-visual-reply indentation, scrollbar hidden (RTL vertical line fix).*
