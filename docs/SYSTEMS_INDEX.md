...
- `company_saved_candidates.rating / priority / tags / follow_up_at / save_source` — حقول تصنيف موسّعة
- `appointments.pipeline_entry_id` — ربط اختياري بإدخال Pipeline
- `auth.py → _migrate_pipeline_schema_v1()` — migration إضافية idempotent
- `server.py → on_startup()` — استدعاء المigration عند التشغيل

**Details:** PR-1 — Additive Schema (هذا الملف § 38) · `ARCHITECTURE_FOUNDATION.md` (قواعد F1–F13) · اختبارات pr-1-01 إلى pr-1-10e في `test_post_comments.py`
