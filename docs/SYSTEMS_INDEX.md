...
**Purpose:** طبقة توجيه معمارية خفيفة تجلس بين طلب صاحب المشروع وبين F31 — تُحدِّد نطاق الطلب، تُصنِّف نوع التغيير، تُعيِّن النظام المالك الأساسي (Primary Owner) والأنظمة الداعمة (Supporting Owners)، تُحدِّد الحد الأدنى من الوثائق اللازمة للقراءة، وتُصدر حكماً (PROCEED / STOP / DISCUSS) مع نطاق التنفيذ.
...
**Details:** `docs/CHANGE_ROUTER.md` — 18 قسماً: Constitutional Position · CRS-01 Routing Engine (Scope Detection → Change Types → Primary/Supporting Owner → Required Reading) · CRS-02 Architectural Sanity Check (10 أسئلة) · System Gap Contract · Shared System First · CRS-03 Credit Control (TINY/MEDIUM/ARCHITECTURAL) · Context Reuse · Ambiguity/Confidence (HIGH/MEDIUM/LOW) · Impact Matrix (10 محاور) · Mobile/API-First Guard · Notification Routing · User Intent ≠ Implementation Method · Audit Mode / Execution Mode · No Auto-Fix · Documentation Discipline · Standard Output Format · Mandatory Architectural Opinion · Cross-references (14 نظاماً حاكماً)
**Do not recreate:** CRS هو Router/Orchestration Layer فقط — ليس Source of Truth لأي نظام آخر. لا تُنشئ ملف "routing" أو "change-guide" مستقل. لا تُعيد نقل عقود الأنظمة إلى هذا الملف (قاعدة الزر تبقى في BUTTONS.md). CRS لا يُعدِّل Runtime تلقائياً — وظيفته Route · Inspect · Classify · Recommend · Verdict فقط. ترتيب الأولوية: `ARCHITECTURE_FOUNDATION → CRS → SYSTEMS_INDEX → Governing System → Runtime`.
**Cross-references:** `ARCHITECTURE_FOUNDATION.md` F31 (System Routing Before Implementation — CRS يجلس فوقه) · `docs/SYSTEMS_INDEX.md` (CRS يستخدمه كـ Primary Lookup لتحديد المالك) · 14 نظاماً مُعرَّفاً في جدول Cross-references داخل `docs/CHANGE_ROUTER.md`.
