...
**Purpose:** 1-level reply threading per-post comments on company posts. Auth required to create/edit/delete. Server-side permission enforcement. Soft delete. XSS-safe rendering via `_renderCommentBody(bodyEl, text, mentionName, mentionTwId)`. `comments_count` returned with every post. Reply threading: `reply_to_comment_id` nullable FK, max depth=1 enforced server-side. Replies grouped under parent on load (`_cmtRenderComments`), inserted after last sibling on send (`_cmtInsertReply`). `.pc-cmt-visual-reply` driven by `reply_to_comment_id`. Portal ⋮ menu (`position:fixed`), auto-resize textarea, relative Arabic time. **Author links:** avatar + name are `<a href="/u/{author_tw_id}">` elements; @mention is `<a href="/u/{reply_to_author_tw_id}">` when tw_id available (reply only), `<span>` otherwise (free mention V1). **Mention dropdown:** portal `#pc-cmt-mention-menu`, candidates `{name, tw_id, avatar}` from DOM (`data-author-tw-id` on items + `companyState.profile`), max 6, keyboard nav, XSS-safe.
**Source of Truth:** `company_post_comments` table (incl. `reply_to_comment_id` column) · `auth.py → get_company_post_comments / create_company_post_comment / update_company_post_comment / delete_company_post_comment` (returns `reply_to_author_tw_id`) · `GET /company/posts/{post_id}/comments` · `POST/PATCH/DELETE /company/posts/comments/{comment_id}` · `static/company/company.posts.js` (`_cmtBuildItem` with author `<a>` links, `_renderCommentBody(bodyEl, text, mentionName, mentionTwId)`, `_cmtCollectMentionCandidates` returns `{name, tw_id, avatar}`, `_cmtFilterMentionCandidates`, `_cmtOpenMentionMenu` with avatar+name, all other comment functions) · `static/company/company.css` (`.pc-cmt-mention-*`, `a.pc-cmt-author`, `a.pc-cmt-ava`, `a.pc-cmt-mention`)
**Details:** `CLAUDE.md → Post Comments System Rules` · `ARCHITECTURE.md §65`
**Do not recreate:** Max depth=1 — server resolves deeper replies to root parent. Do NOT create a separate reply endpoint — same `POST /comments` accepts `reply_to_comment_id`. `_renderCommentBody(bodyEl, text, mentionName, mentionTwId)` — 4 args always; handles compound author names; creates `<a>` when `mentionTwId` truthy. Author/mention links ALWAYS use `/u/{tw_id}` — never numeric id, never legacy routes. Free @mention (not reply) = `<span>` only in V1. `_cmtCollectMentionCandidates` returns `{name, tw_id, avatar}` objects (not strings); uses `companyState.profile` path (NOT `companyState.full_name`). One mention portal on body — do NOT duplicate. `innerHTML` forbidden for any candidate name or API text.
...
*Last updated: 2026-07-07 — reflects systems as of PR #386–#394 + feat/reply-threading-v1 + feat/comment-mention-autocomplete + feat/comment-author-links. §22c updated: reply_to_author_tw_id in API (GET + CREATE), author avatar/name as <a href="/u/tw_id">, @mention as <a> when reply_to_author_tw_id available, _renderCommentBody 4-arg signature, candidate objects {name,tw_id,avatar}, avatar in dropdown, companyState.profile path fix.*
