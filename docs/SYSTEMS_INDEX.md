...
- `auth.py → get_job_applicants(job_id, company_id, view="", page=1, limit=50) → dict` — **(PR-6)** تم توسيع الـ signature · `view=""` → legacy (كل الصفوف، بلا pagination) · `view="applicants"` → `jpe.promoted_at IS NULL` مع COUNT + LIMIT/OFFSET · `view="candidates"` → `jpe.promoted_at IS NOT NULL` مع pagination · `limit` مقيد بـ 1–100 server-side · يُعيد دائماً dict: `{applicants, total, page, limit, view}`
- `server.py → GET /jobs/{job_id}/applicants` — **(PR-6)** query params جديدة: `view`, `page`, `limit` · عند `view=""` (غياب): response legacy `{applicants:[...], count:N}` (backward compat) · عند `view` محدد: response paginated dict · validation: `view` يجب أن يكون `""`, `"applicants"`, أو `"candidates"` (HTTP 400 خلاف ذلك)
