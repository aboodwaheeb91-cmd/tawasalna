...
**Per-job classification contract (feat/candidate-status-per-job):**
...
- Frontend: each job chip in manage panel has `.co-cand-job-status-sel` native `<select>`. `_onSavedChange` delegated handler auto-saves on change. On success, updates `data-job-links` (canonical store) then calls `_renderCandidateJobLinksUI(card, links)` which rebuilds chips, per-job status selects, and picker states atomically. On new job link, same helper is called so chip and select appear immediately without reload. On failure, select rolls back to `data-prev-val` without touching `data-job-links` or the chip.
