...
**Privacy:** Returns `candidate_id, tw_id, full_name, avatar_url, profession, city, country, job_id, status, notes, created_at, updated_at, job_titles[]` only. `job_titles[]` = all job titles the candidate applied to at this company (from `job_applications → jobs`). `job_id` is the first-save origin only (never overwritten). Never returns email, phone, or KYC data. Notes are never searched.
**Pipeline statuses:** `saved | shortlisted | contacted | interview | hired | rejected`. Validated via `VALID_CANDIDATE_STATUSES`. `notes` max 500 chars. `job_id` in `company_saved_candidates` is set only on first insert — never overwritten by subsequent saves or promotes. Per-job accepted state is tracked via `job_applications.status='accepted'`.
**Filters (Phase 7A):** `status`, `job_id`, `unlinked`, `q` (search), `sort` (6 options via `VALID_CANDIDATE_SORTS`). `job_id` + `unlinked=true` are mutually exclusive (400). `q` max 80 chars. When `job_id` filter is active, response includes `per_job_accepted: bool` per candidate (derived from `job_applications.status='accepted'` for that job).
