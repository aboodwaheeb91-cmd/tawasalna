...
| `--color-surface-input` | `rgba(255,255,255,.06)` | خلفية حقول الإدخال والـ select — Phase 2 confirmed consumer: `index.css` |

*ملاحظة:* `--color-surface-input` (`.06`) > `--color-surface-card` (`.03`) — Input أعلى تباين عن الخلفية. القيمة الخاصة مقصودة ومستقلة عن `--color-surface-card`.
