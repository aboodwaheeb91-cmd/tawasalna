...
| **Server Error (على حقل)** | يبقى حتى أول Keystroke | لا حتى صواب القيمة |
...
→ المستخدم يكتب حرفاً واحداً في حقل email → الخطأ يختفي
   (لا ينتظر حتى القيمة تصبح صحيحة)
```
...
```
1. Response.errors[].field موجود → Inline على الحقل المحدد
2. Response.errors[] بدون field  → Form-level (خطأ عام)
...
```js
async function handleSaveError(res) {
  ...
  if (body?.errors?.length) {
    for (const err of body.errors) {
      if (err.field) {
        ...
        // Form-level
        showFormError(err.message || 'حدث خطأ، حاول مجدداً')
      }
    }
    return
  }
  ...
  // Fallback
  showFormError(body?.message || 'حدث خطأ، حاول مجدداً')
}
```
...
| الحدث | ما يُنظَّف |
|-------|-----------|
| **Keystroke في حقل به Server Error** | خطأ هذا الحقل فقط |
...
Server Error على حقل يختفي عند **أول keystroke** في ذلك الحقل — **لا ينتظر** حتى تصبح القيمة صحيحة.
...
**نفس وظائف Validation للـ Add والـ Edit.**

```js
// ✅ صحيح: وظيفة واحدة تتعامل مع الاثنتين
function validateForm(mode) {
  clearAllErrors()
  const errors = []

  const name = nameInput.value.trim()
  if (!name) errors.push({ field: 'name', message: 'الاسم مطلوب' })

  // ... باقي الحقول

  errors.forEach(e => showFieldError(e.field, e.message))
  if (errors.length) focusFirstError()
  return errors.length === 0
}

// نفس الوظيفة للـ Add والـ Edit
saveAddBtn.onclick = () => { if (validateForm('add')) submitAdd() }
saveEditBtn.onclick = () => { if (validateForm('edit')) submitEdit() }
```

### ممنوع

```
❌ validateAddForm() و validateEditForm() منفصلتان بمنطق مكرر
```
...
❌ Server Error يبقى حتى تصبح القيمة صحيحة — يختفي عند أول keystroke
❌ errorEl.innerHTML = message — يُعرِّض لـ XSS
❌ رسائل خطأ بالإنجليزية للمستخدم
❌ كشف DB errors أو Stack traces للمستخدم
❌ Validation منفصل للـ Add وللـ Edit (وظيفتان بمنطق مكرر)
...
|--------|--------|---------|
| `company.main.js` | بعض Server Errors تبقى حتى Blur بدلاً من Keystroke | متوسطة |
