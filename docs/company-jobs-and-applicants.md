...
### أزرار كرت المرشح (V2 — حفظ وتصنيف)

| الزر | الكلاس | الحالة | الوظيفة |
|-----|--------|--------|---------|
| عرض الملف | `.co-app-profile-link` | دائماً | `href=/u/{tw_id}` |
| حفظ وتصنيف ▾ | `.co-classify-btn` | غير مصنَّف | قائمة منسدلة بـ 7 خيارات |
| تعديل التصنيف ▾ | `.co-classify-btn` | مصنَّف (ليس رفض) | تعديل التصنيف الحالي |
| إعادة التصنيف ▾ | `.co-classify-btn.co-classify-btn--reclassify` | مرفوض | إعادة تصنيف المرشح |
| تحديد موعد | `.co-app-sched-btn` | مقابلة (بدون موعد) | يفتح نموذج الموعد |
| فتح الموعد | `.co-app-open-appt-btn` | مقابلة (موعد أُرسل) | يفتح الموعد الموجود |

**خيارات قائمة التصنيف (`.co-ast-opt`):**

| التصنيف | `data-val` | ملاحظة |
|---------|-----------|-------|
| محفوظ | `pending` | الحالة الافتراضية |
| للمراجعة | `viewed` | — |
| مرشح قوي | `accepted` | يستدعي `_execPromote` داخلياً |
| تم التواصل | `contacted` | — |
| مقابلة | `interview` | يُظهر سؤال الآن/لاحقاً |
| تم التوظيف | `hired` | يُخفي خيار "غير مناسب" |
| غير مناسب | `rejected` | `.co-ast-opt--reject`، مخفي عند `hired` |

**سلوك خاص:**
- **مرفوض:** الكرت يبهت (`.co-app-card--rejected`)، يظهر فقط "عرض الملف" + "إعادة التصنيف".
- **مقابلة → الآن:** يفتح نموذج الموعد مباشرة عبر `_openApptModal`.
- **مقابلة → لاحقاً:** يحفظ الحالة بدون إنشاء موعد أو إشعار الموظف.
- **بعد إرسال الموعد:** "تحديد موعد" يُستبدل بـ "فتح الموعد".
- **كل التصنيفات داخلية:** الموظف لا يُشعَر إلا عند إرسال دعوة موعد رسمية.
- **`_reRenderCardFoot(card, statusKey)`** — يعيد بناء كامل footer الكرت بعد أي تصنيف ناجح.

**مهم:** لا يوجد زر "إزالة" في هذا المودال.
