...
*حُدِّث: 2026-07-11 — Final Runtime QA (PR qa): 13 bug fixes — BUG1 (computed status ignored), BUG2 (bogus event), BUG3 (wrong 403), BUG4 (no transaction), BUG5 (completed in dup guard), BUG6 (early complete), BUG7 (messaging on terminal), BUG9 (self-notify close), BUG10 (negative limit), BUG12 (total→count), BUG13 (/send missing type guard), BUG20 (reschedule date), BUG22 (.catch), BUG24 (datetime min), BUG15 (error vs empty). §172 صفحة اختبارات جديدة (43 فحصاً).*
