...
def get_profile(user_id: int):
    ...
    try:
        ...
        # Experiences (verified only for public)
        cur.execute("SELECT * FROM experiences WHERE user_id = %s AND status = 'verified'", (user_id,))
        experiences = cur.fetchall()

        # Educations
        cur.execute("SELECT * FROM educations WHERE user_id = %s AND status = 'verified'", (user_id,))
        educations = cur.fetchall()

        # Courses
        cur.execute("SELECT * FROM courses WHERE user_id = %s AND status = 'verified'", (user_id,))
...
def get_profile_full(user_id: int):
    """للموظف نفسه - يشوف كل شي بما فيه غير الموثق"""
    conn = get_db()
    cur = conn.cursor()
    try:
        cur.execute("SELECT * FROM profiles WHERE user_id = %s", (user_id,))
        ...
        cur.execute("SELECT * FROM experiences WHERE user_id = %s ORDER BY created_at DESC", (user_id,))
        experiences = cur.fetchall()

        cur.execute("SELECT * FROM educations WHERE user_id = %s ORDER BY created_at DESC", (user_id,))
        educations = cur.fetchall()

        cur.execute("SELECT * FROM courses WHERE user_id = %s ORDER BY created_at DESC", (user_id,))
        courses = cur.fetchall()

        cur.execute("SELECT * FROM profile_sections WHERE user_id = %s ORDER BY order_index", (user_id,))
...
def update_profile(user_id: int, data: ProfileUpdate):
    ...
    try:
        cur.execute("""
            INSERT INTO profiles (user_id, title, bio, city, style, color, cv_code)
        ...
        """, (user_id, data.title, data.bio, data.city, data.style, data.color,
              f"TW-EMP-{user_id:05d}"))
        conn.commit()
...
def add_experience(user_id: int, data: ExperienceCreate):
    conn = get_db()
    cur = conn.cursor()
    try:
        cur.execute("""
            INSERT INTO experiences (user_id, title, company, location, start_date, end_date)
            VALUES (%s, %s, %s, %s, %s, %s) RETURNING id
        """, (user_id, data.title, data.company, data.location, data.start_date, data.end_date))
...
def add_education(user_id: int, data: EducationCreate):
    conn = get_db()
    cur = conn.cursor()
    try:
        cur.execute("""
            INSERT INTO educations (user_id, degree, institution, year)
            VALUES (%s, %s, %s, %s) RETURNING id
        """, (user_id, data.degree, data.institution, data.year))
        new_id = cur.fetchone()[0]
        conn.commit()
        return {"status": "added", "id": new_id}
    finally:
...
def add_course(user_id: int, data: CourseCreate):
    conn = get_db()
    cur = conn.cursor()
    try:
        cur.execute("""
            INSERT INTO courses (user_id, name, provider, year)
            VALUES (%s, %s, %s, %s) RETURNING id
        """, (user_id, data.name, data.provider, data.year))
        new_id = cur.fetchone()[0]
        conn.commit()
        return {"status": "added", "id": new_id}
    finally:
...
def request_verification(data: VerificationRequest):
    ...
    try:
        cur.execute("""
            INSERT INTO verification_requests (user_id, item_id, item_type, org_email)
            VALUES (%s, %s, %s, %s)
        """, (data.item_id, data.item_id, data.item_type, data.org_email))
        conn.commit()
