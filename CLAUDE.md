...
### قاعدة فحص الـ PR قبل الرفع (mandatory)

**قبل كل `git push` أو فتح PR — افحص آخر PR متعلق بنفس المهمة أولاً:**

| حالة الـ PR السابق | الإجراء المطلوب |
|--------------------|----------------|
| **مفتوح (open)، غير مدموج** | لا تفتح PR جديد — كمّل التعديلات على نفس الـ branch ونفس الـ PR |
| **مدموج (merged)** | افتح branch جديد + PR جديد، واذكر أنه مبني بعد دمج الـ PR السابق |
| **مغلق بدون دمج (closed)** | لا تبني عليه — اسأل المستخدم أو افتح PR نظيف حسب الحالة |

**ممنوع فتح PR جديد لنفس المهمة إذا الـ PR السابق لسه مفتوح وغير مدموج.**

### تقرير التسليم الإلزامي

في كل تسليم (push أو PR) يجب ذكر:
- رقم الـ PR السابق وحالته (merged / open / closed)
- هل كملت على نفس الـ PR أو فتحت PR جديد
- رقم الـ PR الحالي
